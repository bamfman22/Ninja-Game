...
void ParticleEmitter::start(ofVec3f pos) {
	started = true;
...
void ParticleEmitter::spawn(float time) {
    ...
    //creating a particle with an image if necessary
    if(haveChildImage){
	...
	case RadialEmitter:
	{
		...
		particle.velocity = dir.getNormalized() * speed * 100;
		particle.position.set(posi);
	}
	break;
	case SphereEmitter:
		break;
	case DirectionalEmitter:
		...
		particle.position.set(posi); //use this to move an explosion effect to where the enemy was hit
		break;
	}
